...
从下面**「今日新星候选池」**中筛选今日值得关注的项目。如果新星池为空（0 个），则诚实地写一段说明，不要编造，也不要从常青树池中重复拿项目。按以下品类组织展示，**同类项目只推最好的一个**（比如多个 AI 编排工具只留一个），**AI 产品类不超过 3 个**，给其他品类留空间：
...
3. 每个项目用**三段式介绍**，每段一句话，不加废话：
   - 第一句：一句话定义它是什么（不说技术细节）
   - 第二句：核心卖点——解决什么问题 / 有什么特别
   - 第三句：你为什么要关心（和独立开发者 / 产品思维的直接关联）
